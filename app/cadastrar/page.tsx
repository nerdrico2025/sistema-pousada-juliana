...
export default function CadastrarPage() {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.dataNascimento) {
      ...
      if (age < 18 || age > 120) {
        newErrors.dataNascimento = 'Idade deve estar entre 18 e 120 anos'
      }
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (errorData.error === 'CPF já cadastrado') {
          ...
          alert('Erro ao cadastrar hóspede. Tente novamente.')
        }
      }
    } catch (error) {
    ...
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      >
        <h1 className="text-3xl font-bold text-gray-800">
          <UserPlus className="inline-block mr-3 h-8 w-8 text-green-600" />
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex space-x-4 pt-6">
            ...
            >
              ...
              <span>{loading ? 'Cadastrando...' : 'CADASTRAR HÓSPEDE'}</span>
            </button>
          </div>
        </form>
      </motion.div>
